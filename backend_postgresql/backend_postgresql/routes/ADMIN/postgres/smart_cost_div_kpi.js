...
router.get("/sum-last-status", async (req, res) => {
  try {
    ...
    const result = await query(
      ...
      END AS order_by
    FROM
      public.smart_cost_div_kpi
    WHERE
      NOT EXISTS (
        SELECT 1
        FROM public.smart_cost_div_kpi AS t2
        WHERE t2.cost_type = public.smart_cost_div_kpi.cost_type
          AND TO_DATE(t2.year_month, 'YYYY-MM-DD') > TO_DATE(public.smart_cost_div_kpi.year_month, 'YYYY-MM-DD')
      )
      AND factory = 'A1'
      AND division = $1
    GROUP BY
