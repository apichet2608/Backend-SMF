...
router.get("/data-fix", async (req, res) => {
  try {
    ...
    const result = await query(
      `SELECT
      row_number() OVER (ORDER BY t.production_date) AS id,
      t.production_date,
      t.sendresultdetails_product,
      t.lss_lot_no,
    ...
    FROM
      foxsystem_summary_bylot t
    WHERE
      t.production_date >= $2
      AND DATE_TRUNC('day', t.production_date) <= DATE_TRUNC('day', $3::TIMESTAMP)
      AND t.sendresultdetails_product = $4
    GROUP BY
      t.production_date,
      t.sendresultdetails_product,
      t.lss_lot_no`,
